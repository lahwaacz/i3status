...
void print_wireless_info(yajl_gen json_gen, char *buffer, const char *interface, const char *format_up, const char *format_down) {
    ...
    for (; *walk != '\0'; walk++) {
        ...
        if (BEGINS_WITH(walk + 1, "quality")) {
            if (info.flags & WIRELESS_INFO_FLAG_HAS_QUALITY) {
                if (info.quality_max)
                    outwalk += sprintf(outwalk, "%3d%%", PERCENT_VALUE(info.quality, info.quality_max));
                else
        ...
        if (BEGINS_WITH(walk + 1, "signal")) {
            if (info.flags & WIRELESS_INFO_FLAG_HAS_SIGNAL) {
                if (info.signal_level_max)
                    outwalk += sprintf(outwalk, "%3d%%", PERCENT_VALUE(info.signal_level, info.signal_level_max));
                else
        ...
        if (BEGINS_WITH(walk + 1, "noise")) {
            if (info.flags & WIRELESS_INFO_FLAG_HAS_NOISE) {
                if (info.noise_level_max)
                    outwalk += sprintf(outwalk, "%3d%%", PERCENT_VALUE(info.noise_level, info.noise_level_max));
                else
